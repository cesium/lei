...
public class SwapSolver {
	 ...
	 *
	 * @param jsonString
	 *            a string with textual representation of a json object containing a
	 *            list of exchange requests from students
	 * @return Returns a string with textual representation of a json object
	 *         containing a list of identifiers (can be an empty list if there are
	 *         no possible exchanges)
	 */
	 ...
	 *
	 * @param graph
	 *            the graph in which the cycle is present
	 * @param cycle
	 *            the cycle where the minimum date is to be found
	 * @param minimumCommonDate
	 *            the lower bound for the return value
	...
	public static int minimumDateOfCycle(DefaultDirectedWeightedGraph<String, ERList> graph, List<String> cycle,
		...
		/*
		 * At this point we have the guarantee that minimumDateOfCycle is different than
		 * Integer.MAX_VALUE Every edge in a cycle belongs to a different person,
		 * therefore every edge has a different minimumDateOfEdge If at this point
		 * minimumDateOfCycle were equal to Integer.MAX_VALUE it would mean that every
		 * edge in this cycle would have a minimunDateOfEdge smaller than
		 * minimumCommonDate (which is impossible since the number of iterations is
		 * limited in the "resolveDraw" method)
		 */
	...
	/**
	 * Resolves the cycle to be solved when there is a draw in deciding which cycle
	 * to solve
	 *
	 * @param cycles
	 *            a List of cycles (each cycle is a List of nodes represented by a
	 *            String)
	 * @param graph
	 *            the graph which contains the cycles given
	 * @return Returns the cycle which contains the request(s) made earlier than the
	 *         requests in other cycles
	 */
